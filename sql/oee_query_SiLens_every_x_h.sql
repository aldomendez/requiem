select rownum+(select max(id) from oee_master2) id,':inicio' s_start_dt, ':final' s_end_dt,
--select rownum+(select max(id) from oee_master2) id,to_date(':inicio','yyyy-mm-dd hh24:mi') s_start_dt, to_date(':final','yyyy-mm-dd hh24:mi') s_end_dt,
 a.* , (availability * performance * yield) oee from ( 
...
sample_span_time, round(sum(cycle_time/60),0) total_production_time,
  sum(case when step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then 1 else 0 end) build_qty,
  ...
  -- Performance
  case when sum(case when step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then cycle_time/60 else 0 end) = 0 then 0 else 
  (oee_machine_catalog.IDEAL_CYCLE_TIME/(sum(case when step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then cycle_time/60 else 0 end) /count(serial_num))) end Performance,
  -- Yield
  sum(case when PASS_FAIL = 'P' and step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then 1 else 0 end)/
  case when sum(case when step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then 1 else 0 end) = 0 then .000001 else sum(case when step_name = 'TOSA SUBASSEM3 (SUBASSEM2, SI LENS)' then 1 else 0 end)end Yield
