...
s_end_dt, a.* , (availability * performance * yield) oee from ( 
    ...
    select  
        ...
        from (
          ...
          where case when a.step_name in (select process from oee_machine_catalog where depto = 'ITLA' and machine = 'CYTEST1802' group by process) then a.step_name else 'all' end  = b.process
