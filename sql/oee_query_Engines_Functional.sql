...
select bu_id bu,depto,product,process,display_name system_id,round((to_date(':final','yyyy-mm-dd hh24:mi') - to_date(':inicio','yyyy-mm-dd hh24:mi'))*24*60,0) SAMPLE_TIME_SPAN,
    ...
    sum(cycle_time)/round(((to_date(':final','yyyy-mm-dd hh24:mi') - to_date(':inicio','yyyy-mm-dd hh24:mi'))*24*60)*(1-planed_downtime),0) availability,
...
group by display_name, bu_id, depto, product, process,machine,planed_downtime
)a